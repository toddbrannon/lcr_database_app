...
// Replace 'your_table_name' with the actual name of the table you want to insert into
const tableName = 'EmployeeHours';
const columns = modifiedColumns.map((col) => `\`${col}\``).join(', ');

// Format the values correctly, including handling NULL values and date formatting
const values = modifiedData.map((row) => {
  const rowValues = Object.entries(row).map(([col, value]) => {
    return formatValue(value, col); // Pass the column name along with the value to formatValue
  });
...
const insertQuery = `INSERT INTO ${tableName} (${columns}) VALUES ${values.join(', ')}`;
    ...
    const modifiedData = jsonData.map((row) => {
      ...
      modifiedColumns.forEach((col, index) => {
        ...
        if (typeof value === 'object' && value instanceof Date) {
          // If the value is a date object, convert it to MySQL date format (YYYY-MM-DD)
          modifiedRow[col] = formatDate(value);
        } else if (value === undefined || value === null || value === '') {
          // If the value is undefined, null, or an empty string, set it to null for the database
          modifiedRow[col] = 'NULL';
        } else {
          modifiedRow[col] = value;
        }
      });
      return modifiedRow;
    });
    ...
    const tableName = 'EmployeeHours';
    const columns = modifiedColumns.map((col) => `\`${col}\``).join(', ');
    const values = modifiedData.map((row) => Object.values(row).map(formatValue).join(', '));
    const insertQuery = `INSERT INTO ${tableName} (${columns}) VALUES (${values.join('), (')})`;
